...
@implementation ZBEncryptionRSA
...
+ (NSString *)publicKeyPath:(NSString *)cerName{
    if (cerName.length == 0 || cerName == nil) {
        cerName = @"public_key.der";
    }
   
    NSString *public_key_path = [[NSBundle mainBundle] pathForResource:cerName ofType:nil];
...
+ (void)_rsaTest{
    //原始数据
    NSString *originalString = @"bskj1234";
    
    //第一种方法 * 使用证书进行加密
    //使用.der和.p12中的公钥私钥加密解密
    //如果显示路径为nil大致加密解密失败，参考这里http://blog.csdn.net/huyisu/article/details/44224033
    //如果当时添加是直接拖拽过去，没有真正加入到bundle中，需要在项目设置中，build phases-》copy bundle resources 下面添加自己的数据就可以了。
    NSString *public_key_path = [[NSBundle mainBundle] pathForResource:@"public_key.der" ofType:nil];
    NSString *private_key_path = [[NSBundle mainBundle] pathForResource:@"private_key.p12" ofType:nil];
    
    NSString *encryptStr    = [self encryptString:originalString publicKeyWithContentsOfFile:public_key_path];
    NSString *decryptString = [self decryptString:encryptStr privateKeyWithContentsOfFile:private_key_path password:@"vae"];//私钥创建时候的填写的密码。
    //    NSString *bass = [ZBBase64Object _stringEncodedByBase64:encryptStr];
